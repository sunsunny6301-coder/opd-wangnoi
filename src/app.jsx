...
function mergeArrayById(base, mine, theirs, keyFn, mergeItem) {
  ...
  keys.forEach((k) => {
    ...
    if (mi && ti) out.push(mergeItem ? mergeItem(bi, mi, ti) : mi);   // อยู่ทั้งคู่ → merge (ค่าเริ่ม = ของเรา)
    else if (mi && !ti) { if (!bi) out.push(mi); }                     // มีแต่ของเรา: เพิ่งเพิ่ม→เก็บ / เขาลบ (เคยมีในฐาน)→เอาออก
    else if (!mi && ti) { if (!bi) out.push(ti); }                     // มีแต่ของเขา: เขาเพิ่ง add→เก็บ / เราลบ→เอาออก
  });
  return out;
}
function _visitKey(v) { return `${v.q || ''}|${v.date || ''}`; }
function mergePet(b, m, t) { return { ...m, visits: mergeArrayById((b && b.visits) || [], m.visits || [], t.visits || [], _visitKey) }; }
