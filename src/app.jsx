...
function mergeState(base, mine, theirs) {
  ...
  return {
    ...
    queue: mergeArrayById(base.queue, M.queue, T.queue, (q) => q.q),
    receipts: mergeArrayById(base.receipts, M.receipts, T.receipts, (r) => r.no),
  ...
  useEffect(() => {
    ...
    supa.from('app_state').select('data, updated_at').eq('id', 'main').maybeSingle().then(({ data, error }) => {
      ...
      if (data?.data?.pets && data?.data?.queue) {
        ...
        setBase(data.data);                          // ฐานสำหรับ merge = สิ่งที่โหลดมาจากคลาวด์
      } else {
