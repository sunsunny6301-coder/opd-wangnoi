...
function CatColor({ body, belly, stripe, ear, eye, nose, patches, headPatches, earL, earR }) {
  return (
    <svg viewBox="0 0 64 50" width="42" height="33" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      {/* หาง (โผล่ข้างลำตัว) */}
      <path d="M45,40 C58,39 60,27 51,24 C48,23 47,26 49,27 C55,29 53,36 44,37 Z" fill={body} />
      {/* ลำตัว (แมวนั่งหันหน้า) */}
      <ellipse cx="32" cy="38" rx="14" ry="11" fill={body} />
      {/* แต้มสีบนลำตัว (แมวสามสี/ทักซิโด้) */}
      {(patches || []).map((p, i) => <ellipse key={i} cx={p.cx} cy={p.cy} rx={p.rx} ry={p.ry} fill={p.fill} />)}
      {/* อก/พุงสีอ่อน */}
      <ellipse cx="32" cy="41" rx="8" ry="7.5" fill={belly} />
      {/* ลายทางข้างลำตัว */}
      <g stroke={stripe} strokeWidth="1.6" strokeLinecap="round" opacity=".7" fill="none">
        <path d="M21,34 q-2,3 -1,6" />
        <path d="M43,34 q2,3 1,6" />
      </g>
      {/* ขาหน้า + อุ้งเท้า */}
      <ellipse cx="26" cy="46.5" rx="4.2" ry="3" fill={body} />
      <ellipse cx="38" cy="46.5" rx="4.2" ry="3" fill={body} />
      <g stroke={stripe} strokeWidth="0.8" strokeLinecap="round" opacity=".45">
        <line x1="26" y1="44.5" x2="26" y2="47" />
        <line x1="38" y1="44.5" x2="38" y2="47" />
      </g>
      {/* หัว */}
      <circle cx="32" cy="17" r="13" fill={body} />
      {/* แต้มสีบนหัว (แมวสามสี/ลายขาวกลางหน้า) */}
      {(headPatches || []).map((p, i) => <ellipse key={'h' + i} cx={p.cx} cy={p.cy} rx={p.rx} ry={p.ry} fill={p.fill} />)}
      {/* หูซ้าย/ขวา (ระบุสีแยกได้) */}
      <path d="M22,7 L19,0 L31,5 Z" fill={earL || body} />
      <path d="M42,7 L45,0 L33,5 Z" fill={earR || body} />
      <path d="M24,6 L22.5,1.8 L29,5 Z" fill={ear} />
      <path d="M40,6 L41.5,1.8 L35,5 Z" fill={ear} />
      {/* ลายหน้าผาก (ลาย M ของแมว) */}
      <g stroke={stripe} strokeWidth="1.3" strokeLinecap="round" opacity=".65" fill="none">
        <path d="M32,4 L32,8" />
        <path d="M28.5,5 L29.8,8.4" />
        <path d="M35.5,5 L34.2,8.4" />
      </g>
      {/* แก้ม/ปากสีอ่อน */}
      <ellipse cx="32" cy="22" rx="7" ry="4.6" fill={belly} />
      {/* ตาสองข้าง */}
      <ellipse cx="26" cy="16" rx="3" ry="3.6" fill="#fff" />
      <ellipse cx="38" cy="16" rx="3" ry="3.6" fill="#fff" />
      <circle cx="26" cy="16.4" r="2.2" fill={eye} />
      <circle cx="38" cy="16.4" r="2.2" fill={eye} />
      <circle cx="26" cy="16.6" r="1" fill="#222" />
      <circle cx="38" cy="16.6" r="1" fill="#222" />
      <circle cx="26.8" cy="15.4" r="0.65" fill="#fff" />
      <circle cx="38.8" cy="15.4" r="0.65" fill="#fff" />
      {/* จมูก */}
      <path d="M30.4,20 L33.6,20 L32,22.4 Z" fill={nose} />
      {/* ปาก */}
      <g stroke="#7a6a5a" strokeWidth="0.9" strokeLinecap="round" fill="none" opacity=".65">
        <path d="M32,22.4 q-2,2.4 -4,1" />
        <path d="M32,22.4 q2,2.4 4,1" />
      </g>
      {/* หนวด */}
      <g stroke="#5b5b5b" strokeWidth="0.8" strokeLinecap="round" opacity=".5">
        <line x1="26" y1="21.5" x2="13" y2="19.5" />
        <line x1="26" y1="23" x2="13" y2="24.5" />
        <line x1="38" y1="21.5" x2="51" y2="19.5" />
        <line x1="38" y1="23" x2="51" y2="24.5" />
      </g>
    </svg>
  );
}
...
const CAT_CALICO = {
  ...
  patches: [
    { fill: '#ED9A4C', cx: 24, cy: 37, rx: 7, ry: 6.5 },
    { fill: '#46414C', cx: 40, cy: 40, rx: 6, ry: 5.5 },
  ],
  headPatches: [
    { fill: '#ED9A4C', cx: 39, cy: 12, rx: 6.5, ry: 6.5 },
    { fill: '#46414C', cx: 24, cy: 11, rx: 4.5, ry: 4.5 },
  ],
};
// แมวขาวดำ (ทักซิโด้): ตัวดำ + อก/พุงขาว + ขีดขาวกลางหน้า
const CAT_TUXEDO = {
  body: '#2E2B33', belly: '#FFFFFF', stripe: '#1B1920', ear: '#F4A9B8', eye: '#7CC368', nose: '#E2738F',
  headPatches: [
    { fill: '#FFFFFF', cx: 32, cy: 20, rx: 3.6, ry: 8 },
  ],
};
function FgCats() {
  return (
