...
// เลือกหมวด หรือพิมพ์หมวดใหม่เอง (กด "➕ เพิ่มหมวดใหม่..." → พิมพ์ชื่อได้ · กด ▼ กลับไปเลือกจากรายการ)
function CatSelect({ value, cats, onChange, style }) {
  const [adding, setAdding] = useState(false);
  const inList = cats.includes(value);
  if (adding || (value && !inList)) {
    return (
      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
        <input className="input" value={value} onChange={(e) => onChange(e.target.value)} placeholder="ชื่อหมวดใหม่..." style={style} autoFocus />
        <button type="button" className="btn btn-sm" style={{ fontSize: 11, padding: '3px 6px', flexShrink: 0 }} title="เลือกจากรายการ"
          onClick={() => { setAdding(false); onChange(cats[0] || 'บริการ'); }}>▼</button>
      </div>
    );
  }
  return (
    <select className="select" value={value} style={style}
      onChange={(e) => { if (e.target.value === '__new__') { setAdding(true); onChange(''); } else onChange(e.target.value); }}>
...
function ServiceManagerModal({ services, stock, onAdd, onSaveService, onDeleteService, onUpdateService, onClose }) {
  ...
  // หมวดให้เลือกใน dropdown = หมวดตั้งต้น + หมวดที่เคยเพิ่มเองในข้อมูลจริง (โผล่ให้เลือกครั้งต่อไป)
  const svcCats = useMemo(() => {
    const set = new Set(SVC_CATS);
    all.forEach((s) => { if (s.cat) set.add(s.cat); });
                ...
                if (editId === s.id) {
                  return (
                    <tr key={s.id} style={{ background: 'var(--powder-soft, #f0f4ff)' }}>
                      ...
                      <td>
                        <CatSelect value={editForm.cat} cats={svcCats} onChange={(v) => setEditForm({ ...editForm, cat: v })} style={{ fontSize: 12, padding: '4px 6px' }} />
                      </td>
        ...
        <div style={{ borderTop: '2px solid var(--line)', paddingTop: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
            ...
            <div style={{ width: 140, display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <CatSelect value={newItem.cat} cats={svcCats} onChange={(v) => setNewItem({ ...newItem, cat: v })} />
            </div>
