...
function TaxInvoice({ items, petName, ownerName, method, vatMode, no }) {
  ...
  return (
    <div style={S.page} className="print-sheet">
      {/* ── clinic header ── */}
      <div style={S.center}>
        <img src="assets/logo.jpg" alt="logo" style={{ width: 72, height: 72, borderRadius: '50%' }} />
...
function ReceiptModal({ title, items, petName, ownerName, onClose, onConfirm, confirmLabel, paidAlready, noVat, defaultVatMode }) {
  ...
  const no = useMemo(() => {
    ...
    return `RCP-${thYear}-${String(Date.now()).slice(-5)}`;
  }, []);
  ...
  return (
    ...
    >
      ...
      <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 'var(--radius)', padding: '24px 28px', maxHeight: '70vh', overflowY: 'auto' }}>
        <TaxInvoice
          items={items} petName={petName} ownerName={ownerName}
